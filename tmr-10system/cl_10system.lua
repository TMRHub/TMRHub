...
RegisterNetEvent('qb-10system:client:open')
AddEventHandler('qb-10system:client:open', function()
    SendNUIMessage({action = "open10system"})
    enable = true
end)

RegisterNetEvent('qb-10system:client:close')
AddEventHandler('qb-10system:client:close', function()
    SendNUIMessage({action = "close"})
    enable = false
end)

RegisterNetEvent("qb-voice:setTalkingOnRadio",function(pSrc,talking)
    if Config.ActivePlayers[PlayerJob.name] then
...
AddEventHandler("qb-voice:radioActive",function(talking)
    if Config.ActivePlayers[PlayerJob.name] then
        local id = GetPlayerServerId(PlayerId())
        for i = 1, #lastData do
            if lastData[i].src == id then
...
RegisterNetEvent('qb-10system:client:update')
AddEventHandler('qb-10system:client:update', function(data,job)
...
RegisterNetEvent('qb-10system:client:updateSteel')
AddEventHandler('qb-10system:client:updateSteel', function(index,job, bool)
...
RegisterNetEvent('qb-10system:client:openPanel')
AddEventHandler('qb-10system:client:openPanel', function()
    SendNUIMessage({action = "open"})
    enable = true
    SetNuiFocus(true, true)
end)

RegisterNetEvent('qb-10system:client:sendError')
AddEventHandler('qb-10system:client:sendError', function(text)
...
--     TriggerServerEvent('qb-10system:server:rank',data.rank)
-- end)
...
RegisterCommand("+open10System",function()
    if (Config.ActivePlayers[PlayerJob.name]) then
        TriggerEvent("qb-10system:client:openPanel")
    end
end)
